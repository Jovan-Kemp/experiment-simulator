...
# Marimo app: motion-coherence demo with forward ``DdmObserver`` (ssm-simulators DDM).
...
def _(app_dir, mo, project_root):
    ...
    intro_title = mo.md(
        "## Forward DDM observer: simulated experiment to HSSM pipeline (motion coherence)"
    )
    intro_body = mo.md(
        r"""
...
- **Observer**: ``DdmObserver`` — forward **DDM** via ssm-simulators; signed evidence maps to drift \(v\), boundary \(a\), starting point \(z\), and non-decision time \(t\).
...
<div class="coherence-demo-simulator-info">
  <details>
    ...
    <div class="coherence-demo-simulator-info__panel">
      ...
      <code>DdmObserver</code>: alternative strengths map to signed evidence and trial drift
